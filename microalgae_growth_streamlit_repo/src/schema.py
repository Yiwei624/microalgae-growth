...
signal_timeseries = Table(
    "signal_timeseries", metadata,
    Column("signal_id", Integer, primary_key=True, autoincrement=True),
    Column("experiment_id", Integer, ForeignKey("experiment.experiment_id"), nullable=False),
    Column("time_d", Float, nullable=False),

    Column("absorbance_od", Float),     # OD / absorbance
    Column("cells_per_ml", Float),      # cell density (cells/mL)
    Column("pressure_kPa", Float),      # internal pressure (kPa)
    Column("pH", Float),                # pH (e.g., final pH or measured pH)
    Column("notes", Text),
)
