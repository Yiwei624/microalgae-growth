...
def _get_or_create_media(engine: Engine, row: Dict) -> Optional[int]:
    key_fields = ["name", "mode", "carbon_source", "carbon_conc_gL", "nitrogen_source", "nitrogen_as_N_mM", "phosphorus_as_P_mM", "salinity_gL"]
    with engine.begin() as conn:
        where = []
        for f in key_fields:
...
def _get_or_create_reactor(engine: Engine, row: Dict) -> Optional[int]:
    key_fields = ["reactor_type", "working_volume_L", "light_path_cm", "mixing_rpm", "aeration_vvm", "gas_co2_percent", "gas_flow_Lmin"]
    with engine.begin() as conn:
        where = []
        for f in key_fields:
            if f in row and row[f] is not None and row[f] != "":
...
def resolve_fk_ids(engine: Engine, exp_row: Dict) -> Dict:
    ...
    if g and s:
        with engine.connect() as conn:
            ex = conn.execute(select(organism.c.organism_id).where(and_(organism.c.genus == g, organism.c.species == s, organism.c.strain == stn))).fetchone()
            if ex:
