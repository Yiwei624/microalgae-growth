...
from .schema import metadata, study, organism, media, reactor, experiment, outcome, timeseries, signal_timeseries, provenance
...
def insert_signal_timeseries(engine: Engine, df: pd.DataFrame, source_label: str = "", uploaded_by: str = "upload") -> int:
    """
    Insert raw signal time series (OD, cells/mL, pressure, pH).
    Expects either experiment_id or exp_code.
    """
    inserted = 0
    df = df.copy()
    if "exp_code" in df.columns:
    ...
    with engine.begin() as conn:
        for _, row in df.iterrows():
            ...
            for c in signal_timeseries.c.keys():
                if c in ("signal_id", "experiment_id", "time_d"):
                    continue
            ...
            conn.execute(signal_timeseries.insert().values(**data))
            inserted += 1
